...
/// Tracks which endpoints are currently being explored, so that the periodic
/// site-explorer loop and ad-hoc `RefreshEndpointReport` calls don't probe the same BMC at the same
/// time.
///
...
/// duplicate probe is harmless (and the write is still guarded by optimistic concurrency). If
/// `nico-api` is ever scaled to multiple active replicas, this would no longer dedupe across them.
...
impl EndpointExplorationLocks {
    ...
    /// task is already exploring it (the periodic loop or another ad-hoc refresh).
