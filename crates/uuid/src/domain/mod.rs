...
mod tests {
    ...
    fn test_string_round_trip() {
        ...
        let parsed = DomainId::from_str(&as_string).expect("failed to parse");
        assert_eq!(id, parsed);
    }
    ...
    fn test_json_round_trip() {
        ...
        let parsed: DomainId = serde_json::from_str(&json).expect("failed to deserialize");
        assert_eq!(id, parsed);
    ...
    #[test]
    fn test_ordering() {
        let id1 = DomainId::from(uuid::Uuid::nil());
        let id2 = DomainId::from(uuid::Uuid::max());
        assert!(id1 < id2);
    }

    #[test]
    fn test_default() {
        let id = DomainId::default();
    ...
    fn test_hash_consistency() {
        ...
        let id1 = DomainId::from(uuid);
        let id2 = DomainId::from(uuid);
    ...
    fn test_debug_includes_type_name() {
        let id = DomainId::from(uuid::Uuid::nil());
        let debug = format!("{:?}", id);
        assert!(debug.contains("DomainId"));
    }
}
