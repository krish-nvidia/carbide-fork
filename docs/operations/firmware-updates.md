...
| Host ConnectX-7 InfiniBand firmware needs to meet an approved baseline. | [ConnectX-7 host firmware](firmware-updates/connectx-7.md) | Publish the catalog and firmware files to each environment, then request host reprovisioning. An assigned host also needs reboot approval. |
